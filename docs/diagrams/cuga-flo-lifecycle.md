...
sequenceDiagram
    ...
    rect rgb(230, 240, 255)
        ...
        FA->>Bridge: register_engine(LangGraphWorkflowEngine(), registry)
    ...
    rect rgb(255, 235, 235)
        Note over Eng,DA: ENGINE EXECUTION — Agentic Gateway Node
        Eng->>Eng: _create_gateway_node(gateway_id)
        Note right of Eng: Builds ControlPointContext:<br/>available_flows — outgoing BPMNFlows<br/>current_state — engine-held FS<br/>execution_history — engine-held path<br/>(no task_instruction)
    ...
    rect rgb(245, 230, 255)
        ...
        Note right of FA: Checks hook.condition against state<br/>If policy: _llm_hook_decision(hook, ctx)<br/>Reads hook.policy (durable) + ctx<br/>Returns HookResult.to_dict()
        FA-->>Bridge: HookResult dict {action, skip_to_node, swap_nodes, ...}
        Bridge-->>Eng: HookResult dict
        Note right of Eng: Enforces action_permissions<br/>Returns Command(goto=target)
    end
